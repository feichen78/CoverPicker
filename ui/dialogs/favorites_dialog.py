...
class FavImageLabel(QLabel):
    ...
    def set_image_size(self, w: int, h: int):
        if w != self.current_img_w or h != self.current_img_h:
            ...
            self.setFixedSize(w + 2, h + 2); self._update_display_pixmap()
    def _update_display_pixmap(self):
        if self.original_pixmap.isNull():
            self.display_pixmap = QPixmap(self.current_img_w, self.current_img_h)
            self.display_pixmap.fill(QColor(60,60,60)); self.update(); return
        w = max(10, self.current_img_w); h = max(10, self.current_img_h)
        scaled = self.original_pixmap.scaled(w, h, Qt.KeepAspectRatio, Qt.SmoothTransformation)
        self.display_pixmap = QPixmap(w, h); self.display_pixmap.fill(QColor(30,30,30))
        painter = QPainter(self.display_pixmap)
        x = (w - scaled.width())//2; y = (h - scaled.height())//2
        painter.drawPixmap(x, y, scaled); painter.end(); self.update()
    def paintEvent(self, event):
        if self.display_pixmap.isNull():
            self._update_display_pixmap()
            if self.display_pixmap.isNull():
                painter = QPainter(self); painter.fillRect(self.rect(), QColor(60,60,60)); painter.end(); return
        painter = QPainter(self); painter.drawPixmap(0, 0, self.display_pixmap)
        painter.setRenderHint(QPainter.Antialiasing)
        if self.is_selected:
            painter.setBrush(QBrush(QColor(33,150,243))); painter.setPen(Qt.NoPen); painter.drawEllipse(6,6,14,14)
        painter.setPen(QColor(255,165,0,230))
        painter.setFont(QFont("Arial",9,QFont.Bold))
        time_str = format_time(self.time_sec)
        painter.drawText(4, self.height() - 6, time_str)
        painter.end()
    ...
    def set_selected(self, selected: bool): self.is_selected = selected; self.update()
...
class FavoritesDialog(QDialog):
    def __init__(self, favorites: List[dict], video_name: str, export_base: str, video_path: str, parent=None):
        ...
        self._image_labels: List[FavImageLabel] = []; self._grid_widgets: List[QWidget] = []
        self._loaded = False; self._last_cols = 0; self._all_items: List[dict] = []
        self.MAX_COLS = 9; self.MIN_IMG_W = 160
        self.MIN_IMG_H = int(self.MIN_IMG_W * 0.75); self.IMG_ASPECT = 0.75
        ...
        self.resize(1100, 900); self.setMinimumSize(800, 700)
        self.setup_ui()
    def setup_ui(self):
        main_layout = QVBoxLayout(self); main_layout.setContentsMargins(10,10,10,10); main_layout.setSpacing(6)
        top_bar = QHBoxLayout()
        self.info_label = QLabel("共 0 张收藏截图"); self.info_label.setFont(QFont("Arial",12,QFont.Bold))
        top_bar.addWidget(self.info_label); top_bar.addStretch()
        self.select_all_btn = QPushButton("☑ 全选"); self.select_all_btn.clicked.connect(self.select_all)
        self.select_all_btn.setEnabled(False); top_bar.addWidget(self.select_all_btn)
        self.deselect_all_btn = QPushButton("☐ 取消全选"); self.deselect_all_btn.clicked.connect(self.deselect_all)
        self.deselect_all_btn.setEnabled(False); top_bar.addWidget(self.deselect_all_btn)
        self.unfavorite_btn = QPushButton("☆ 取消收藏"); self.unfavorite_btn.clicked.connect(self.unfavorite_selected)
        self.unfavorite_btn.setEnabled(False); top_bar.addWidget(self.unfavorite_btn)
        self.export_btn = QPushButton("📥 导出选中"); self.export_btn.clicked.connect(self.export_selected)
        self.export_btn.setEnabled(False); top_bar.addWidget(self.export_btn)
        self.zoom_btn = QPushButton("🔍 细选"); self.zoom_btn.clicked.connect(self.zoom_selected)
        ...
        self.content_widget = QWidget(); self.content_widget.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
        self.content_layout = QVBoxLayout(self.content_widget)
        self.content_layout.setSpacing(4); self.content_layout.setContentsMargins(0,2,0,2)
        self.content_layout.setAlignment(Qt.AlignTop)
        self.scroll.setWidget(self.content_widget); main_layout.addWidget(self.scroll,1)
        bottom_bar = QHBoxLayout()
        self.selected_label = QLabel("已选: 0 张"); bottom_bar.addWidget(self.selected_label)
        bottom_bar.addStretch(); main_layout.addLayout(bottom_bar)
    ...
    def _get_item_key(self, item: dict) -> tuple:
        return (item.get('segment','A'), item.get('time', 0))
    def select_all(self):
        self.selected_keys = {self._get_item_key(item) for item in self._all_items}
    ...
    def _update_selection_states(self):
        for idx, label in enumerate(self._image_labels):
            if idx < len(self._all_items):
                key = self._get_item_key(self._all_items[idx])
                label.set_selected(key in self.selected_keys)
    ...
    def showEvent(self, event):
        ...
        if not self._loaded:
            QTimer.singleShot(100, self.load_favorites); self._loaded = True
    def resizeEvent(self, event: QResizeEvent):
        super().resizeEvent(event); self._resize_timer.start(100)
    def _on_resize_timeout(self):
        if not self._image_labels: return
        vw, vh = self._get_viewport_size()
        if vw < 100 or vh < 100: return
        total_items = len(self._image_labels)
        cols, img_w, img_h = self._calculate_optimal_layout(vw, vh, total_items)
        if cols != self._last_cols:
            self._last_cols = cols; self.load_favorites()
        else:
            for label in self._image_labels: label.set_image_size(img_w, img_h)
            self._update_grid_cols(cols); self._update_inner_widgets(img_w, img_h)
    ...
    def _calculate_optimal_layout(self, viewport_width: int, viewport_height: int, total_items: int) -> Tuple[int, int, int]:
        if viewport_width < 100 or viewport_height < 100:
            viewport_width = 1000; viewport_height = 700
        spacing = 4; padding = 6; title_height = 26; seg_spacing = 4
        grouped = self._get_seg_items(); seg_count = len(grouped)
        ...
        best_cols = max_cols_by_width; best_img_w = self.MIN_IMG_W; best_img_h = self.MIN_IMG_H
        found_valid = False
        for cols in range(max_cols_by_width, 0, -1):
            rows = (total_items + cols - 1) // cols
            if rows > max_rows_by_height: continue
            found_valid = True
            ...
            img_w = max(self.MIN_IMG_W, img_w); img_h = max(self.MIN_IMG_H, img_h)
            max_img_w = (viewport_width - padding * 2 - spacing * (cols - 1)) // cols
            if img_w > max_img_w:
                img_w = max_img_w; img_h = int(img_w * self.IMG_ASPECT)
                img_h = max(self.MIN_IMG_H, img_h); img_w = max(self.MIN_IMG_W, img_w)
            if img_w > best_img_w:
                best_cols = cols; best_img_w = img_w; best_img_h = img_h
        if not found_valid:
            ...
            best_img_w = max(self.MIN_IMG_W, best_img_w); best_img_h = max(self.MIN_IMG_H, best_img_h)
        best_img_w = max(self.MIN_IMG_W, best_img_w); best_img_h = max(self.MIN_IMG_H, best_img_h)
    ...
    def _update_grid_cols(self, cols: int):
        for widget in self._grid_widgets:
            if widget and widget.layout() and isinstance(widget.layout(), QGridLayout):
                ...
                for col in range(grid_layout.columnCount()): grid_layout.setColumnStretch(col, 0)
                for col in range(cols): grid_layout.setColumnStretch(col, 1)
    ...
    def _get_seg_items(self) -> Dict[str, List[dict]]:
        ...
        for item in self.favorites:
            ...
            if seg not in grouped: grouped[seg] = []
    ...
    def load_favorites(self):
        ...
        self._image_labels = []; self._grid_widgets = []; self._all_items = self._flatten_items()
        self.selected_keys.clear()
        if not self.favorites:
            label = QLabel("暂无收藏截图")
            label.setAlignment(Qt.AlignCenter); label.setStyleSheet("font-size:16px;color:#999;padding:40px;")
            self.content_layout.addWidget(label)
            self.info_label.setText("共 0 张收藏截图")
            self.select_all_btn.setEnabled(False); self.deselect_all_btn.setEnabled(False)
            self.unfavorite_btn.setEnabled(False)
            return
        grouped = self._get_seg_items(); total = len(self.favorites)
        self.info_label.setText(f"共 {total} 张收藏截图")
        vw, vh = self._get_viewport_size()
        if vw < 100 or vh < 100: vw = 1000; vh = 700
        cols, img_w, img_h = self._calculate_optimal_layout(vw, vh, total)
        self._last_cols = cols; inner_height = img_h + 22
        for seg_label in sorted(grouped.keys()):
            ...
            grid_widget = QWidget(); grid_widget.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
            grid_layout = QGridLayout(grid_widget); grid_layout.setSpacing(4); grid_layout.setContentsMargins(2,2,2,2)
            for col in range(self.MAX_COLS):
                grid_layout.setColumnStretch(col, 1 if col < cols else 0)
            rows = (len(items) + cols - 1) // cols
            for row in range(rows): grid_layout.setRowStretch(row, 1)
            for pos, item in enumerate(items):
                ...
                cell_widget = QWidget(); cell_widget.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
                cell_layout = QHBoxLayout(cell_widget); cell_layout.setSpacing(0); cell_layout.setContentsMargins(0,0,0,0)
                cell_layout.setAlignment(Qt.AlignCenter)
                inner_widget = QWidget(); inner_widget.setSizePolicy(QSizePolicy.Fixed, QSizePolicy.Fixed)
                inner_widget.setFixedSize(img_w + 2, inner_height)
                inner_widget.setProperty("is_inner_container", True)
                inner_layout = QVBoxLayout(inner_widget); inner_layout.setSpacing(2); inner_layout.setContentsMargins(0,0,0,0)
                img_label = FavImageLabel(pixmap, item['time'])
                img_label.setObjectName(f"{seg_label}_{pos}")
                img_label.clicked.connect(partial(self.on_fav_item_click, img_label))
            ...
            self.content_layout.addWidget(grid_widget); self._grid_widgets.append(grid_widget)
        self.selected_label.setText("已选: 0 张")
        self.export_btn.setEnabled(False)
        self.unfavorite_btn.setEnabled(False)
        self.select_all_btn.setEnabled(len(self.favorites)>0)
        self.deselect_all_btn.setEnabled(False)
    def on_fav_item_click(self, label: FavImageLabel):
        idx = self._image_labels.index(label)
        if idx >= len(self._all_items):
            return
    ...
    def unfavorite_selected(self):
        if not self.selected_keys:
            QMessageBox.information(self, "提示", "请先选中要取消收藏的截图。"); return
        to_remove = []
        ...
        reply = QMessageBox.question(self, "确认取消收藏", f"确定要取消收藏选中的 {len(to_remove)} 张截图吗？", QMessageBox.Yes|QMessageBox.No)
        if reply != QMessageBox.Yes: return
        removed = 0
        for seg_label, time_sec in to_remove:
            self.favorites = [f for f in self.favorites if not (f.get('segment')==seg_label and abs(f.get('time',0)-time_sec)<0.01)]
            ...
            if self.parent_view:
                ...
                for item in items:
                    if abs(item['time'] - time_sec) < 0.01:
                        item['favorite'] = False; break
            removed += 1
        if self.parent_view:
            self.parent_view.favorites = [f for f in self.parent_view.favorites if not any(f.get('segment')==seg_label and abs(f.get('time',0)-time_sec)<0.01 for seg_label,time_sec in to_remove)]
            self.parent_view._update_fav_count(); self.parent_view._update_seg_buttons()
            if self.parent_view.video_path: self.parent_view._update_video_list_icon(self.parent_view.video_path)
    ...
    def export_selected(self):
        if not self.selected_keys:
            QMessageBox.information(self, "提示", "请先选中要导出的截图。"); return
        export_paths = []
        for idx, item in enumerate(self._all_items):
            key = self._get_item_key(item)
            if key in self.selected_keys:
        ...
        if not export_paths:
            QMessageBox.warning(self, "警告", "选中的截图文件不存在。"); return
        video_name = os.path.splitext(os.path.basename(self.video_path))[0]
        if not video_name: video_name = "favorites"
        ...
        for time_sec, src_path, seg_label in export_paths:
            ...
            try:
                shutil.copy2(src_path, dest_path); exported += 1
                for item in self.favorites:
                    if item.get('segment')==seg_label and abs(item.get('time',0)-time_sec)<0.01:
                        item['exported'] = True; break
            except Exception as e: print(f"导出失败 {src_path}: {e}")
        QMessageBox.information(self, "导出完成", f"成功导出 {exported} 张截图到:\n{export_dir}")
        if self.parent_view:
            for seg_label, pos in self.selected_keys:
                items = [item for item in self.favorites if item.get('segment')==seg_label]
                if pos < len(items):
                    item = items[pos]
                    if self.parent_view.video_path:
                        self.parent_view._mark_favorite_exported(seg_label, item['time'])
            ...
            if self.parent_view.video_path:
                ...
                if self.parent_view.video_id: self.parent_view._save_state_to_db()
        self.selected_keys.clear()
        self._update_buttons()
        self.load_favorites()
    def zoom_selected(self):
        print(f"[DEBUG] zoom_selected 被调用, selected_keys数量: {len(self.selected_keys)}")
        count = len(self.selected_keys)
        if count != 1:
            QMessageBox.information(self, "提示", f"细选只能针对单张截图，请只选中一张截图。当前选中了 {count} 张。")
            print(f"[DEBUG] 显示提示: count={count}")
            return
        selected_key = next(iter(self.selected_keys))
        seg_label, time_sec = selected_key
        item = None
        for it in self._all_items:
            if self._get_item_key(it) == selected_key:
                item = it; break
        if not item:
            QMessageBox.warning(self, "警告", "数据错误。"); return
        ...
        if not controller:
            QMessageBox.warning(self, "警告", "无法获取主控制器。"); return
        seg_idx = 0
        segments = controller.get_segments()
        ...
        dlg = ZoomDialog(controller=controller, seg_label=seg_label, seg_idx=seg_idx, pos=pos, center_time=time_sec, level=1, parent=self, source="favorites", original_fav_item=item, original_fav_time=time_sec)
        dlg.exec()
        self.load_favorites()