...
class ZoomDialog(QDialog):
        ...
        for pos in selected_positions:
            ...
            if not self.controller.db.is_favorite(video_id, self.seg_label, timestamp_ms):
                self.controller.db.add_favorite(
                    video_id, self.seg_label, timestamp_ms,
                    item['path'], is_exported=False
                )
            ...
            try:
                ...
                if self.controller.video_id:
                    ...
                    self.controller.db.update_favorite_exported(
                        self.controller.video_id, self.seg_label, timestamp_ms
                    )
    ...
    def replace_selected(self):
        ...
        reply = QMessageBox.question(
            ...
            f"原时间: {self._format_time(self.center_time)} → 新时间: {self._format_time(item['time'])}",
            QMessageBox.Yes | QMessageBox.No
        )
        if reply != QMessageBox.Yes:
            return

        success = self.controller.replace_screenshot(
            self.seg_label, self.pos, item['time'], item['path'], self.center_time
        )
        if success:
            self.controller._save_state_to_db()
            self.controller._notify_data_changed()
