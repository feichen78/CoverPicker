...
class Database:
    ...
    def _init_db(self) -> None:
        ...
        if rows: conn.commit()
        try: cursor.execute("CREATE INDEX IF NOT EXISTS idx_videos_file_id ON videos(file_id)")
        except: pass
        cursor.execute("""
    ...
    def get_or_create_video(self, file_path: str, file_name: str, duration: int, resolution: str = "", file_size: int = 0, modified_time: int = 0) -> int:
        ...
        if row:
            ...
            if row['file_size'] != file_size or row['modified_time'] != modified_time:
                cursor.execute("UPDATE videos SET file_size = ?, modified_time = ?, duration = ?, resolution = ? WHERE id = ?", (file_size, modified_time, duration, resolution, vid))
                conn.commit()
            return vid
        cursor.execute("SELECT id, file_size, modified_time FROM videos WHERE file_path = ?", (file_path,))
        row = cursor.fetchone()
        if row:
            vid = row['id']; cursor.execute("UPDATE videos SET file_id = ? WHERE id = ?", (file_id, vid)); conn.commit(); return vid
        cursor.execute("INSERT INTO videos (file_id, file_path, file_name, duration, resolution, file_size, modified_time) VALUES (?,?,?,?,?,?,?)", (file_id, file_path, file_name, duration, resolution, file_size, modified_time))
    ...
    def get_video_id_by_path_or_file_id(self, file_path: str, file_size: int, modified_time: int) -> Optional[int]:
        ...
        row = cursor.fetchone()
        if row:
            cursor.execute("UPDATE videos SET file_id = ? WHERE id = ?", (file_id, row['id'])); conn.commit(); return row['id']
        return None
    def update_video_state(self, video_id: int, is_viewed: Optional[bool] = None, is_starred: Optional[bool] = None, is_exported: Optional[bool] = None) -> None:
        conn = self._get_conn(); cursor = conn.cursor()
        updates = []; params = []
        if is_viewed is not None: updates.append("is_viewed = ?"); params.append(1 if is_viewed else 0)
        if is_starred is not None: updates.append("is_starred = ?"); params.append(1 if is_starred else 0)
        if is_exported is not None: updates.append("is_exported = ?"); params.append(1 if is_exported else 0)
        if not updates: return
        updates.append("last_edited = strftime('%s','now')"); params.append(video_id)
        cursor.execute(f"UPDATE videos SET {', '.join(updates)} WHERE id = ?", params); conn.commit()
    def get_all_videos(self) -> List[Dict]:
        conn = self._get_conn(); cursor = conn.cursor()
        cursor.execute("SELECT id, file_path, file_name, duration, resolution, file_size, is_viewed, is_starred, is_exported FROM videos ORDER BY file_name")
        return [dict(row) for row in cursor.fetchall()]
    def get_or_create_segment(self, video_id: int, segment_label: str, time_start: int, time_end: int) -> int:
        conn = self._get_conn(); cursor = conn.cursor()
        cursor.execute("SELECT id, excluded_ranges FROM segments WHERE video_id = ? AND segment_label = ?", (video_id, segment_label))
        row = cursor.fetchone()
        if row: return row['id']
        cursor.execute("INSERT INTO segments (video_id, segment_label, time_start, time_end, excluded_ranges) VALUES (?,?,?,?,?)", (video_id, segment_label, time_start, time_end, '[]'))
        conn.commit(); return cursor.lastrowid
    def update_segment_state(self, video_id: int, segment_label: str, is_viewed: Optional[bool] = None, has_starred: Optional[bool] = None, has_exported: Optional[bool] = None, excluded_ranges: Optional[List[Tuple[float, float]]] = None) -> None:
        conn = self._get_conn(); cursor = conn.cursor()
        updates = []; params = []
        if is_viewed is not None: updates.append("is_viewed = ?"); params.append(1 if is_viewed else 0)
        if has_starred is not None: updates.append("has_starred = ?"); params.append(1 if has_starred else 0)
        if has_exported is not None: updates.append("has_exported = ?"); params.append(1 if has_exported else 0)
        if excluded_ranges is not None: updates.append("excluded_ranges = ?"); params.append(json.dumps(excluded_ranges))
        if not updates: return
        params.extend([video_id, segment_label])
        cursor.execute(f"UPDATE segments SET {', '.join(updates)} WHERE video_id = ? AND segment_label = ?", params); conn.commit()
    def get_segment_state(self, video_id: int, segment_label: str) -> Optional[Dict]:
        conn = self._get_conn(); cursor = conn.cursor()
        cursor.execute("SELECT is_viewed, has_starred, has_exported, excluded_ranges FROM segments WHERE video_id = ? AND segment_label = ?", (video_id, segment_label))
        row = cursor.fetchone()
        if row:
            result = dict(row)
            if result.get('excluded_ranges'):
                try: result['excluded_ranges'] = json.loads(result['excluded_ranges'])
                except: result['excluded_ranges'] = []
            else: result['excluded_ranges'] = []
            return result
        return None
    def get_all_segments(self, video_id: int) -> List[Dict]:
        conn = self._get_conn(); cursor = conn.cursor()
        cursor.execute("SELECT segment_label, is_viewed, has_starred, has_exported, excluded_ranges FROM segments WHERE video_id = ? ORDER BY segment_label", (video_id,))
        results = []
        for row in cursor.fetchall():
            item = dict(row)
            if item.get('excluded_ranges'):
                try: item['excluded_ranges'] = json.loads(item['excluded_ranges'])
                except: item['excluded_ranges'] = []
            else: item['excluded_ranges'] = []
            results.append(item)
        return results
    def add_favorite(self, video_id: int, segment_label: str, timestamp_ms: int, thumbnail_path: str = "", thumbnail_name: str = "", is_exported: bool = False) -> int:
        conn = self._get_conn(); cursor = conn.cursor()
    ...
    def remove_favorite(self, video_id: int, segment_label: str, timestamp_ms: int) -> None:
        conn = self._get_conn(); cursor = conn.cursor()
        cursor.execute("DELETE FROM favorites WHERE video_id = ? AND segment_label = ? AND timestamp_ms = ?", (video_id, segment_label, timestamp_ms))
        conn.commit()
    def update_favorite_exported(self, video_id: int, segment_label: str, timestamp_ms: int) -> None:
        conn = self._get_conn(); cursor = conn.cursor()
        cursor.execute("UPDATE favorites SET is_exported = 1 WHERE video_id = ? AND segment_label = ? AND timestamp_ms = ?", (video_id, segment_label, timestamp_ms))
        conn.commit()
    def update_favorite_path(self, video_id: int, segment_label: str, timestamp_ms: int, new_path: str) -> None:
        ...
        cursor.execute("UPDATE favorites SET thumbnail_path = ?, thumbnail_name = ? WHERE video_id = ? AND segment_label = ? AND timestamp_ms = ?", (new_path, thumbnail_name, video_id, segment_label, timestamp_ms))
        conn.commit()
    def clear_favorites(self, video_id: int) -> None:
        conn = self._get_conn(); cursor = conn.cursor()
        cursor.execute("DELETE FROM favorites WHERE video_id = ?", (video_id,)); conn.commit()
    def get_favorites(self, video_id: int) -> List[Dict]:
        conn = self._get_conn(); cursor = conn.cursor()
        cursor.execute("SELECT segment_label, timestamp_ms, thumbnail_path, thumbnail_name, is_exported, created_at FROM favorites WHERE video_id = ? ORDER BY segment_label, timestamp_ms", (video_id,))
        return [dict(row) for row in cursor.fetchall()]
    def is_favorite(self, video_id: int, segment_label: str, timestamp_ms: int) -> bool:
        conn = self._get_conn(); cursor = conn.cursor()
        cursor.execute("SELECT 1 FROM favorites WHERE video_id = ? AND segment_label = ? AND timestamp_ms = ?", (video_id, segment_label, timestamp_ms))
    ...
    def restore(self, backup_path: str) -> Tuple[bool, str]:
        try:
            if not os.path.exists(backup_path): return False, f"备份文件不存在: {backup_path}"
            if self._conn: self._conn.close(); self._conn = None
            old_path = self.db_path + ".old"
            if os.path.exists(self.db_path): shutil.copy2(self.db_path, old_path)
    ...
    def get_backup_history(self, backup_dir: str, limit: int = 20) -> List[Dict]:
        ...
        for f in os.listdir(backup_dir):
            if f.startswith("coverpicker_backup_") and f.endswith(".db"):
                ...
                backups.append({'path': file_path, 'name': f, 'size': stat.st_size, 'time': datetime.fromtimestamp(stat.st_mtime).strftime("%Y-%m-%d %H:%M:%S"), 'mtime': stat.st_mtime})
        backups.sort(key=lambda x: x['mtime'], reverse=True)
        return backups[:limit]
    def vacuum(self) -> None:
        conn = self._get_conn(); conn.execute("VACUUM"); conn.commit()
    def delete_video(self, video_id: int) -> None:
        conn = self._get_conn(); cursor = conn.cursor()
        cursor.execute("DELETE FROM videos WHERE id = ?", (video_id,)); conn.commit()