...
class SegmentController:
    ...
    def load_excluded_ranges_from_db(self):
        if not self.video_id or not self.segments:
            return
        seg_label = self.segments[self.current_seg_index][0]
    ...
    async def load_video(self, video_path: str) -> bool:
        ...
        self.video_id = self.db.get_or_create_video(
            video_path, file_name, int(duration), "",
            int(os.path.getsize(video_path)), int(os.path.getmtime(video_path))
        )

        for label, start, end in self.segments:
            self.db.get_or_create_segment(
                self.video_id, label, int(start), int(end)
            )
    ...
    async def _load_segment(self, seg_idx: int, restore_locks: bool = True, randomize: bool = False):
        ...
        if not self.video_path:
            print("[DEBUG] _load_segment 返回: video_path 为空")
            return
        
        if not self.segments:
            print("[DEBUG] _load_segment 返回: segments 为空")
            return
        
        if seg_idx < 0 or seg_idx >= len(self.segments):
            print(f"[DEBUG] _load_segment 返回: seg_idx={seg_idx} 超出范围")
            return

        current_task = asyncio.current_task()
        if current_task and current_task.cancelled():
            print(f"[DEBUG] 分段 {seg_idx} 加载被取消")
            return
            ...
            if not item.get('favorite', False):
                ...
                if self.video_id:
                    ...
                    if not self.db.is_favorite(self.video_id, seg_label, timestamp_ms):
                        self.db.add_favorite(
                            ...
                            item.get('path', ''), item.get('exported', False)
                        )
                ...
                if record_history:
                    self._push_action(Action(
                        'favorite', self.video_id, seg_label, timestamp_ms,
                        old_state, new_state
                    ))
    ...
    def _unfavorite_selected_impl(self, seg_label: str, positions: List[int], record_history: bool) -> int:
        ...
        for pos in positions:
            ...
            if item.get('favorite', False):
                ...
                if record_history:
                    self._push_action(Action(
                        'unfavorite', self.video_id, seg_label, timestamp_ms,
                        old_state, new_state
                    ))
    ...
    def replace_screenshot(self, seg_label: str, pos: int, new_time: float, new_path: str, old_time: float) -> bool:
        ...
        if favorite and self.video_id:
            ...
            self.db.add_favorite(
                self.video_id, seg_label, new_timestamp_ms,
                new_temp_path, is_exported=exported
            )
            ...
            if pos < len(items):
                ...
                if not item.get('locked', False):
                    ...
                    if record_history and self.video_id:
                        timestamp_ms = int(item['time'] * 1000)
                        self._push_action(Action(
                            'lock', self.video_id, seg_label, timestamp_ms,
                            old_state, new_state
                        ))
        if count > 0:
            self._notify_data_changed()
            ...
            if pos < len(items):
                ...
                if item.get('locked', False):
                    ...
                    if record_history and self.video_id:
                        timestamp_ms = int(item['time'] * 1000)
                        self._push_action(Action(
                            'unlock', self.video_id, seg_label, timestamp_ms,
                            old_state, new_state
                        ))
        if count > 0:
            self._notify_data_changed()
    ...
    def auto_clean_cache(self, threshold_gb: float = 5.0) -> Tuple[int, float]:
        ...
        for root, dirs, files in os.walk(self.temp_dir):
            for f in files:
                ...
                try:
                    files_info.append((file_path, os.path.getmtime(file_path), os.path.getsize(file_path)))
                except OSError:
                    pass
            ...
            for item in items:
                if item.get('favorite', False):
                    ...
                    if current_key not in existing_set:
                        self.db.add_favorite(
                            self.video_id, seg_label, timestamp_ms,
                            item.get('path', ''), is_exported=item.get('exported', False)
                        )
                    else:
                        self.db.update_favorite_exported(self.video_id, seg_label, timestamp_ms)
